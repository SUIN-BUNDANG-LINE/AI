...
class EditWithChatService:
    ...
    def edit_survey(self, request: EditSurveyWithChatRequest):
        parser_to_survey = PydanticOutputParser(pydantic_object=Survey)
        formatted_edit_prompt = edit_total_survey_prompt.format(user_prompt=request.user_prompt, user_survey_data=request.survey_data.json())
        ...
        editted_result = self.__chat_edit_survey_data(edit_survey_data_content)
        parsed_result = parser_to_survey.parse(editted_result)
        return parsed_result

    def edit_section(self, request: EditSectionWithChatRequest):
        parser_to_section = PydanticOutputParser(pydantic_object=Section)
        formatted_edit_prompt = edit_total_survey_prompt.format(user_prompt=request.user_prompt, user_survey_data=request.survey_data.json())
    ...
    def edit_question(self, request: EditQuestionWithChatRequest):
        parser_to_question = PydanticOutputParser(pydantic_object=Question)
        formatted_edit_prompt = edit_total_survey_prompt.format(user_prompt=request.user_prompt, user_survey_data=request.survey_data.json())
    ...
    def __chat_edit_survey_data(self, edit_survey_data_content: _EditSurveyDataContent):
